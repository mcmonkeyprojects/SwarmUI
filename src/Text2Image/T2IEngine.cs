...
namespace SwarmUI.Text2Image
{
    ...
    public static class T2IEngine
    {
        ...
        public record class PostGenerationEventParams(MediaFile File, T2IParamInput UserInput, Action RefuseImage, string ComfyNodeId = null);
        ...
        public class ImageOutput
        {
            ...
            /// <summary>The backend node id that produced this output, if known.</summary>
            public string ComfyNodeId;
        }
        ...
        public static async Task CreateImageTask(T2IParamInput user_input, string batchId, Session.GenClaim claim, Action<JObject> output, Action<string> setError, bool isWS, float backendTimeoutMin, Action<ImageOutput, string> saveImages, bool canCallTools)
        {
            ...
            void handleFileOutput(ImageOutput img)
            {
                ...
                bool refuse = false;
                PostGenerateEvent?.Invoke(new(img.File, copyInput, () => refuse = true, img.ComfyNodeId));
                if (refuse)
                {
