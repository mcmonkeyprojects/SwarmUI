                ...
                if (output is not null)
                {
                    ...
                    else
                    {
                        ...
                        else if (isReceivingOutputs)
                        {
                            ...
                            takeOutput(new T2IEngine.ImageOutput() { File = new Image(output[preBytes..], mediaType), IsReal = isReal, ComfyNodeId = string.IsNullOrEmpty(currentNode) ? null : currentNode, GenTimeMS = firstStep == 0 ? -1 : (Environment.TickCount64 - firstStep) });
                        }
                        else
                        {
