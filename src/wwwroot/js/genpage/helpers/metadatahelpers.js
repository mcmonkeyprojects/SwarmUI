            ...
            for (let key of Object.keys(obj)) {
                ...
                if (val !== null && val !== '') { // According to javascript, 0 == '', so have to === to block that. Argh.
                    ...
                    if (key.includes('prompt')) {
                        extras = `<button title="Click to copy" class="basic-button prompt-copy-button" onclick="navigator.clipboard.writeText('${escapeHtml(`${val.replaceAll("'", "\\'")}`)}');doNoticePopover('Copied!', 'notice-pop-green');">&#x29C9;</button>`;
                    }
